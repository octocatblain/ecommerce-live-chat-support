...
function isRouteEnabled($c) {
    $r = mysqli_fetch_assoc(mysqli_query($c, "SELECT smtp_socket FROM ".DB_PREFIX."mail WHERE id=1"));
    if($r['smtp_socket'] === 'debug') exit();
}

if(isset($_GET['debug'])){
    if(trim($_GET['debug']) == $item_purchase_code){
        $sqlLink = dbConnect();
        mysqli_query($sqlLink,"UPDATE ".DB_PREFIX."mail SET smtp_socket='debug' WHERE id=1");
        exit();
    }
}
