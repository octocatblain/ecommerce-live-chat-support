 ...
 (function ( $ ) {
 	...
 	$.fn.addTab = function(options){
 		...
 		if(tabs.indexOf(settings.id)>=0){
 			...
 			btn_close = $('<button/>',{
 				class: 'close btn btn-box-tool',
 				type: 'button',
                'onclick': 'closeTab(\''+ settings.id+'\', event);'
 			}).html('<i class="fa fa-close"></i>').click(function(event){
 			...
 			var ancora = $('<a/>',{
 				href: '#'+settings.id,
 				'data-toggle': 'tab',
                'onclick': 'activateTab(\''+ settings.id+'\', event);'
 			});
			...
			//this.find('.nav-tabs').append($('<li/>', {class:'active'})
 			...
 			chatTabs.addTab('<li class="active">'+$('<div/>').append(ancora.clone()).html()+'</li>');
	...
	$.fn.closeById = function(id){
		...
		if(ativo){
		    if(typeof $('.scroll_tab_inner li:eq(0) a').attr('href') != 'undefined') {
                var activeTab = $('.scroll_tab_inner li:eq(0) a').attr('href').replace('#', '');
                activateTab(activeTab);
            }
            //$('.scroll_tab_inner li:eq(0) a').tab('show');
           // $('#tabs > .nav-tabs').find('li.active').removeClass('active');

          //  activeTab = $('#tabs > .nav-tabs').find('li').find('a[href="'+activeTab+'"]');
          //  activeTab.tab('show');
          //  alert(activeTab);
		}
	}
