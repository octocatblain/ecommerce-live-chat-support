...
function addLang($langData,$con){
    ...
    $query = "UPDATE ".DB_PREFIX."interface SET available_languages='$allLang' WHERE id='1'";
    mysqli_query($con,$query);
    $langData[2] = 'lang_'.$langData[2];
    $query1 = "ALTER TABLE ".DB_PREFIX."lang ADD $langData[2] text";
...
function removeLang($code,$con){
    ...
    $query = "UPDATE ".DB_PREFIX."interface SET available_languages='$newLangs' WHERE id='1'";
    mysqli_query($con,$query);
    $query1 = "ALTER TABLE ".DB_PREFIX."lang DROP COLUMN lang_$code";
    mysqli_query($con,$query1);
    return true;
}
...
function langStatusChange($code, $status, $con){
    ...
    foreach($allLangs as $la){
        if(in_array($code,$la, true)){
            $newLangs[] = array($la[0],$la[1],$la[2],$la[3],$la[4],$status,$la[6]);
        }else
            $newLangs[] = $la;
    }
    $newLangs = serialize($newLangs);
...
function langUpdateAll($code, $langArr, $con){
    ...
    foreach($allLangs as $la){
        if(in_array($code,$la, true))
            $newLangs[] = array($langArr[0],$la[1],$langArr[1],$langArr[2],$la[4],$langArr[3],$langArr[4]);
        else
            $newLangs[] = $la;
    }
    $newLangs = serialize($newLangs);
    ...
    if($code != $langArr[1])
        mysqli_query($con,"ALTER TABLE `".DB_PREFIX."lang` CHANGE `lang_$code` `lang_".$langArr[1]."` text");
...
function getAvailableLanguages($con){
     ...
     $query = mysqli_query($con, "SELECT * FROM ".DB_PREFIX."interface WHERE id='1'");
...
function getLangData($langCode,$con){
    if(strlen($langCode) == 2){
        ...
        if($langOkay){
            $langCodeData = mysqli_query($con, "SELECT code, lang_$langCode FROM ".DB_PREFIX."lang");
...
function getDefaultLang($con){
    $query =  "SELECT * FROM ".DB_PREFIX."interface where id='1'";
...
function customAdminLangLink($link='', $langCode, $subPath, $return=false){
    $langShortCode = '';
    $link = str_replace('/'.ADMIN_DIR_NAME,'',$link);
    if(!defined('BASEURL'))
        die('Base URL not set!');
    if(defined('LANG_SHORT_CODE'))
        $link = str_replace('/'.LANG_SHORT_CODE, '' , $link);
    $link = str_replace($subPath, '' , $link);
    if($return)
        return BASEURL.ADMIN_DIR_NAME.'/'.$langCode.$link;
    else
        echo BASEURL.ADMIN_DIR_NAME.'/'.$langCode.$link;
}